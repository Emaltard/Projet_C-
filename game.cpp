...
Game:: Game(int nbjoueur){
    ...
    //Affichage de l'emplacement du tir suivant
    ...
    emplacement_du_futur_tir=new QGraphicsEllipseItem(tir_x-10,tir_y-10,10*2,10*2);
    emplacement_du_futur_tir->setBrush(QBrush(QColor(Qt::white)));
    emplacement_du_futur_tir->setPen(QPen(Qt::DotLine));
    emplacement_du_futur_tir->setZValue(-1);
    scene->addItem(emplacement_du_futur_tir);

    ligne_char_emplacement_tir=new QGraphicsLineItem(tir_x-10,tir_y-10, currentChar->getX()+(25/2), currentChar->getY()+(25/2));
    ligne_char_emplacement_tir->setPen(QPen(Qt::DotLine));
    ligne_char_emplacement_tir->setZValue(-1);
    scene->addItem(ligne_char_emplacement_tir);
...
void Game::controler_ia(QList<obstacle *> list_ob)
{
    ...
    if (collidingObstacle(*currentChar,list_ob) && depla<10) {
        for (int i=0;i<list_ob.length();i++) {
            ...
            else if (list_ob[i]->getType()==3){ // Si c'est une fissure il continue a bouger
